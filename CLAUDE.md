...
This is a Slidev theme package (`@luocfprime/slidev-theme-ustc`). The published surface is `components/`, `layouts/`, `public/`, `setup/`, `styles/`, `utils/`, `global-top.vue`, and `vite.config.ts`. Everything else (`example.md`, `examples/`, `tests/`, `docs/`) is local-only and excluded from npm.
...
**`utils/`**: `layoutHelper.ts` — shared CSS-in-JS helpers for background, margin, and grid resolution. `numbering.ts` — figure/table auto-numbering. `markdown.ts` — markdown-it helpers. `defaults.ts` — shared prop defaults.
